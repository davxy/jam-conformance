            ...
            if isinstance(value, str) and "{TARGET_SOCK}" in value:
                ...
                for sub_key, sub_value in value.items():
                    if isinstance(sub_value, str) and "{TARGET_SOCK}" in sub_value:
                        processed_dict[sub_key] = sub_value.format(TARGET_SOCK=TARGET_SOCK)
...
Use 'info all' to see available targets.
    ...
    parser.add_argument(
        '--os',
        choices=['linux', 'macos'],
        help='Target OS (default: auto-detected)'
    )
...
def target_supports_os(target: str, os_name: str) -> bool:
    if target not in TARGETS:
        return False
    target_obj = TARGETS[target]
    if not target_obj.supports_os(os_name):
        print(f"Error: No {os_name} version available for {target}", file=sys.stderr)
        return False
    return True

def get_target_file(target: str, os_name: str) -> Optional[str]:
    if target not in TARGETS:
        return None
    return TARGETS[target].get_file(os_name)

def post_actions(target: str, os_name: str) -> bool:
    ...
    target_obj = TARGETS[target]
    if target_obj.post:
        subprocess.run(target_obj.post, shell=True, check=True)
    else:
...
def clone_github_repo(target: str, os_name: str, repo: str) -> bool:
    with tempfile.TemporaryDirectory() as temp_dir:
        subprocess.run(['git', 'clone', f'https://github.com/{repo}', '--depth', '1', temp_dir], check=True)
...
def get_github_release(target: str, os_name: str) -> bool:
    ...
    if target_obj.clone == 1:
        print(f"Info: No release file specified for {target} on {os_name}, cloning repository instead")
...
def run_docker_image(target: str) -> None:
    ...
    docker_cmd = ['docker', 'run', '--rm', '--name', target,
                  '--user', f"{os.getuid()}:{os.getgid()}",
                  '--platform', 'linux/amd64',
                  '--cpuset-cpus', f"0-{MAX_CORES}",
                  '--cpu-shares', '2048',
                  '--memory', '8g',
                  '--memory-swap', '8g',
                  '--shm-size', '1g',
                  ...
                  '--cap-add', 'IPC_LOCK',
                  '-v', '/tmp:/tmp',
                  '-v', f"{Path(__file__).parent}/targets/{target}/latest:/jam"]
    ...
    current_os = get_os()
    if current_os == "linux":
        priority_cmd = ['sudo', 'chrt', '-f', '99', 'nice', '-n', '-20', 
                       'ionice', '-c1', '-n0', 'taskset', '-c', f"0-{MAX_CORES}"]
...
def handle_info_action(target: str, os_name: str) -> bool:
    ...
    if target == "all":
        ...
        for target_name in get_available_targets():
            if not handle_info_action(target_name, os_name):
                success = False
        return success
    else:
        if target in TARGETS:
            target_obj = TARGETS[target]
            print_target_info(target, target_obj, os_name)
            return True
        else:
            print(f"Error: Target {target} not found")
            return False
...
def handle_get_action(target: str, os_name: str) -> bool:
    ...
    if target == "all":
        ...
        if not failed_targets:
            ...
            print(f"Failed to download the following targets: {' '.join(failed_targets)}")
            total_targets = len(available_targets)
            successful = total_targets - len(failed_targets)
            print(f"Successfully downloaded: {successful} out of {total_targets} targets")
            return False
...
def print_target_info(target_name: str, target_obj: Target, os_name: str) -> None:
    """Print detailed information about a target."""
    print(f"\n=== {target_name.upper()} ===")
    print(f"Name: {target_name}")
    
    if target_obj.repo:
        print(f"Repository: https://github.com/{target_obj.repo}")
    
    if target_obj.image:
        print(f"Docker Image: {target_obj.image}")
    
    if target_obj.clone:
        print(f"Clone Mode: {'Yes' if target_obj.clone == 1 else 'No'}")
    
    if target_obj.file:
        print("Files:")
        for os_key, file_path in target_obj.file.items():
            print(f"  {os_key}: {file_path}")
    
    if target_obj.cmd:
        if isinstance(target_obj.cmd, dict):
            print("Commands:")
            for os_key, cmd in target_obj.cmd.items():
                print(f"  {os_key}: {cmd}")
        else:
            print(f"Command: {target_obj.cmd}")
    
    if target_obj.args:
        print(f"Arguments: {target_obj.args}")
    
    if target_obj.env:
        print(f"Environment: {target_obj.env}")
    
    # Show OS support
    supported_oses = []
    for os_check in ['linux', 'macos']:
        if target_obj.supports_os(os_check):
    ...
    elif target_obj.is_docker_target():
        # Check if Docker image exists locally
        try:
            subprocess.run(['docker', 'image', 'inspect', target_obj.image], 
                          check=True, capture_output=True)
